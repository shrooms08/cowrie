...
| **Mocked** | The **anchor / fiat rails** (no real money; a fixed, *exposed* FX rate stands in for SEP-38/31/24). **Token transfer-in & merchant payout** (no real USDC moves on deposit/spend). The **ASP admin** (a server route holds the admin key and vouches identities — stands in for the attestation service). The **blocklist root** is admin-set, not a live SMT. **Fixed denominations {1,5,10,50}**, no change notes. **Dev-only trusted setup** (single-contributor, throwaway entropy). "**Seedless**" = a local Stellar key auto-provisioned + friendbot-funded (production would use passkey/social login). |
| **Production needs** | A proper **pool/ASP indexer** (the demo reconstructs trees from recent RPC events + local overlay; public RPC retains only ~recent ledgers). A **real Stellar anchor** via SEP-24/31/38. **Passkey / social login** instead of a local key. A **real USDC SAC** wired into deposit/withdraw with auth. An **audited multi-party trusted-setup ceremony**. A live on-chain **blocklist SMT**. |
